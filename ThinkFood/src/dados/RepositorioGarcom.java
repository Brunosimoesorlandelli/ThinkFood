...
public class RepositorioGarcom implements IRepositorioGarcom {
		...
		public boolean cadastrar(Funcionario_Garcon p) throws Exception {
			String query = "insert into fornecedor (nome, cpf, dataNasc, cep, salario, numero)values(?,?,?,?,?)";
		...
		public Funcionario_Garcon procurar(String cnpj) throws Exception {
			...
			while (rs.next()) {
				f = new Funcionario_Garcon(rs.getString("cpf"), rs.getString("nome"),
						rs.getDate("data_nasc"), rs.getString("fornecedor_end"), rs.getInt("fone"));
			}
			return f;
